...
def _attention_prefill_ragged(
    ...
    def batch_prefill_ragged_kv(  # pylint: disable=too-many-branches
        ...
        rope_scale: T.float32,
        rope_theta: T.float32,
        q_rope_pos_offset: T.int32,
        k_rope_pos_offset: T.int32,
        attn_score_scaling_factor: T.float32
    ):
        batch_size = T.int32(is_size_var=True)
                        ...
                        with T.block("attn"):
                            ...
                            for li, lj in T.grid(tile_x, tile_y):
                                with T.block("Q_load"):
                                    ...
                                    if cur_L < qo_len:
                                        Q_smem[i, j] = T.if_then_else(
                                            rotary_mode == 1,
                                            _rope(q, q_rope_pos_offset, d, rope_theta, rope_scale, (cur_L, cur_H_qo, j), dtype, rope_scaling),
                                            q[cur_L, cur_H_qo, j]
                                        )
                                    else:
                                        Q_smem[i, j] = 0.0
                                    ...
                                    with T.block("K_load"):
                                        ...
                                        if cur_L < kv_len:
                                            K_smem[i, j] = T.if_then_else(
                                                ...
                                                _rope(k, k_rope_pos_offset + cur_L, d, rope_theta, rope_scale, (cur_L, by, j), dtype, rope_scaling),
                            ...
                            for li, lj in T.grid(tile_x, tile_y):
                                with T.block("O_store"):
                                    i, j = T.axis.remap("SS", [li, lj])
                                    # cur_L: T.int32 = q_indptr[b_idx] + (LH_start + i) // group_size
                                    cur_L: T.int32 = (LH_start + i) // group_size
                                    cur_H_qo: T.int32 = by * group_size + (LH_start + i) % group_size
                                    # if cur_L < q_indptr[b_idx + 1]:
                                    if cur_L < qo_len:
                                        output[cur_L, cur_H_qo, j] = O_local[i, j] / d_smem[i]
