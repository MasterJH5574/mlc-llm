...
class Engine {
 ...
 public:
  ...
  explicit Engine(int max_single_sequence_length, String tokenizer_path,
                  String kv_cache_config_json_str, Optional<PackedFunc> request_stream_callback,
...
class EngineModule : public ModuleNode {
 public:
  ...
  void InitPacked(TVMArgs args, TVMRetValue* rv) {
    ...
    this->engine_ =
        std::make_unique<Engine>(max_single_sequence_length, tokenizer_path,
                                 kv_cache_config_json_str, request_stream_callback, model_infos);
  }
  ...
  Optional<PackedFunc> GetRequestCallback() { return GetEngine()->request_stream_callback_; }
  /*! \brief Set the engine request callback function. */
  void SetRequestCallback(Optional<PackedFunc> request_stream_callback) {
    GetEngine()->request_stream_callback_ = std::move(request_stream_callback);
  }
